    ...
    def create_nodes_and_relationships(cls, csv_path, uri, username, password, database):
        ...
        with connector.driver.session() as session:
            for index, row in df.iterrows():
                ...
                rating = row.get('Rating')  # Assuming 'Rating' is the column containing star ratings
                ...
                if rating:
                    map_rating_query = """
                    MATCH (s:Song {name: $song})
                    MERGE (r:Rating {stars: $rating})
                    MERGE (s)-[:RATED]->(r)
                    """
                    session.run(map_rating_query, song=song, rating=rating)
                    print(f"Song mapped to rating: {song} - {rating}")
