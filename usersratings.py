    ...
    def create_nodes_and_relationships(cls, csv_path, uri, username, password, database):
        ...
        with connector.driver.session() as session:
            ...
            for index, row in df.iterrows():
                ...
                # Map ratings to song using stars
                for i, rating in enumerate(ratings, start= 1):
                    if rating:
                        stars = '*' * rating  # Convert rating to stars
                        map_rating_query = f"""
                        MATCH (s:Song {{name: $song}})
                        MERGE (u:User {{username: $username}})
                        MERGE (s)-[:RATED {{stars: $stars}}]->(u)
                        """
                        session.run(map_rating_query, song=song, username=f"User {i}", stars=stars)
                        print(f"Rating {rating} mapped to Song {song} and User {i} as {stars}")
